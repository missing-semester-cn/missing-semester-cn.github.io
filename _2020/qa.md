...
一般来说，vim 和 Python 是两个不常规的数据整理工具。对于某些复杂的多行转换，vim 宏是非常有用的工具。你可以记录一系列操作，并根据需要重复执行多次，例如，在“编辑器”一节的 [讲义](/2020/editors/#macros)（去年 [视频](/2019/editors/)）中，有一个示例是使用 vim 宏将 XML 格式的文件转换为 JSON。
