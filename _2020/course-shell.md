...
但是我们却常常忘记，这一点不仅适用于利用程序去求解问题，也同样适用于我们使用计算机的方式。 
在从事与计算机相关的工作时，我们有很多触手可及的工具可以帮助我们更高效地解决问题。
然而，我们中的大多数人实际上只利用了这些工具中极少的一部分；我们常常只是死记硬背一些如咒语般的命令，或是当卡住的时候，盲目地从网上复制粘贴。
...
取而代之的是我们会在需要的时候使用 `sudo` 命令。顾名思义，它的作用是让您能够以 su（super user 或 root 的简写）的身份执行一些操作。
