...
正如上一讲所讨论的，大多数 Shell 不只是个程序启动器。
实际上它们提供了一套完整的编程语言，包含各种常见模式和抽象。
但与主流编程语言不同的是，Shell 脚本的一切设计都是为了运行程序，并让程序之间能以简单、高效的方式相互通信。

具体来说，Shell 脚本深受各种*约定（conventions）*的影响。CLI 程序要想在 Shell 环境中良好协作，就必须遵循一些通用模式。
接下来介绍理解命令行程序运作方式所需的核心概念，以及使用、配置这些程序的常见约定。
...
在 Shell 脚本内部，可以通过特殊语法访问这些参数。
`$1` 访问第一个参数，`$2` 到 `$9` 以此类推。`$@` 以列表形式访问所有参数，`$#` 获取参数个数。`$0` 则是程序本身的名称。

对于大多数程序，参数通常由*选项（flags）*和普通字符串组成。
选项的特征是前面带 `-` 或 `--`。
它们通常是可选的，用来修改程序的行为。
比如 `ls -l` 改变了 `ls` 的输出格式。

你会看到长名称的双横线选项如 `--all`，以及单字母的单横线选项如 `-a`。
很多情况下两种格式等价，比如 `ls -a` 和 `ls --all` 效果一样。
单横线选项通常可以合并，所以 `ls -l -a` 和 `ls -la` 也等价。
选项顺序通常无所谓，`ls -la` 和 `ls -al` 结果相同。
有些选项非常常用，随着你对 Shell 越来越熟悉，会本能地用到它们，比如 `--help`、`--verbose` 和 `--version`。
...
这种语法糖初看可能没啥必要，但配合*通配符扩展（globbing）* 使用就威力十足了。
...
最常见的通配模式包括 `*`（匹配零个或多个任意字符）、`?`（恰好匹配一个字符），以及花括号。
...
一个很重要的观察是，这三个程序其实是同时运行的。
也就是说，Shell 并不是先执行 `cat`，再执行 `grep`，最后执行 `uniq`。
相反，三个程序会一起被启动，Shell 把 `cat` 的输出接到 `grep` 的输入，再把 `grep` 的输出接到 `uniq` 的输入。
使用管道运算符 `|` 时，Shell 操作的就是从前一个程序流向后一个程序的数据流。
...
stdout 是最常见的输出，也是默认会通过管道传给下一个命令的。
stderr 是另一条流，程序用它输出警告和其他问题信息，这样这些内容就不会被管道中的下一个命令当成正常数据处理。
...
在 bash 里，给变量赋值写成 `foo=bar`，读取变量值用 `$foo`。
注意 `foo = bar` 是非法语法，因为 Shell 会把它解析成调用程序 `foo`，并传入参数 `['=', 'bar']`。
在 Shell 中，空格的作用就是分隔参数。
这个行为一开始不太符合直觉，要特别记住。

Shell 变量没有类型，本质上全都是字符串。
还要注意，单引号和双引号在 Shell 中并不能互换。
`'` 包起来的是字面量字符串，不展开变量、不做命令替换、也不处理转义序列；`"` 包起来的则会。
...
`ls` 的输出（即 stdout）会被存进变量 `$files`，后面可以继续使用。
`$files` 中保留了 `ls` 输出的换行符，所以 `grep` 等程序能把每一项当成独立的一行来处理。

另一个类似但没那么出名的特性是*进程替换（process substitution）*。`<( CMD )` 会执行 `CMD`，把输出存入临时文件，再用临时文件名替换掉 `<()`。
当某个命令要求通过文件而不是 stdin 传值时，这就很有用了。
比如，`diff <(ls src) <(ls docs)` 会展示 `src` 和 `docs` 两个目录中文件列表的差异。

每当 Shell 程序调用另一个程序时，它会顺带传过去一组变量，这些就是*环境变量（environment variables）*。
在 Shell 里可以通过 `printenv` 查看当前环境变量。
想显式给某个命令传环境变量，可以在命令前面直接加赋值：
...
默认情况下，Shell 脚本返回退出码 0。
按惯例，0 表示一切正常，非 0 表示执行中遇到了问题。
要主动返回非零退出码，使用内建命令 `exit NUM`。
上一条命令的返回码可以通过 `$?` 获取。

Shell 里有布尔运算符 `&&` 和 `||`，分别表示 AND 和 OR。
但与普通编程语言不同，Shell 中这两个运算符基于程序的返回码来工作。
它们也都是[短路](https://en.wikipedia.org/wiki/Short-circuit_evaluation)运算符。
也就是说，你可以根据前一个命令是否成功来条件性地执行后续命令，「成功」的判断标准就是返回码是否为 0。看几个例子：
...
有时你需要在程序执行到一半时打断它，比如某条命令跑太久了。
最简单的做法是按 `Ctrl-C`，命令通常就会停下来。
但这背后到底发生了什么？为什么有时又停不掉？
...
信号是一种特殊的通信机制。
当进程收到信号时，它会暂停当前执行来处理信号，并根据信号携带的信息决定是否改变后续执行流程。所以信号本质上是一种*软件中断（software interrupt）*。
...
`SIGINT` 和 `SIGQUIT` 通常与终端交互操作有关，如果你想更通用地请求进程「优雅退出」，一般用 `SIGTERM`。
...
要让 `ssh` 允许在远程服务器上执行命令，就必须先证明自己有这个权限。
认证可以通过密码，也可以通过 SSH 密钥。
基于密钥的认证使用公钥密码学，在不泄露私钥的前提下向服务器证明客户端持有私钥。
密钥认证更方便也更安全，因此通常应优先使用。
注意，私钥（过去常见 `~/.ssh/id_rsa`，现在更常见 `~/.ssh/id_ed25519`）本质上等同于你的密码，一定要妥善保管，绝不要泄露。
...
在命令行里工作时，你经常想同时做不止一件事。
比如把编辑器和正在运行的程序并排放在一起。
虽然多开终端窗口也能做到，但终端复用器是更灵活的方案。

像 [`tmux`](https://www.man7.org/linux/man-pages/man1/tmux.1.html) 这样的终端复用器，允许你通过窗格（pane）和标签页（tab）来复用终端窗口，从而高效地同时操作多个 Shell 会话。
此外，终端复用器还支持把当前会话分离（detach）出去，之后再重新附着（attach）回来。
正因如此，它在远程机器上尤其方便，通常不再需要 `nohup` 之类的技巧。
...
很多命令行程序通过纯文本配置文件来配置，这类文件通常被称为 _dotfiles_。
之所以叫这个名字，是因为文件名通常以 `.` 开头（如 `~/.vimrc`），默认在 `ls` 列表里是隐藏的。

> Dotfiles 也是 shell 世界里的另一种约定。文件名前面的点号就是为了在列目录时把它们“隐藏”起来（没错，又是一个约定）。

Shell 本身就是通过 dotfiles 配置的程序之一。启动时会读取多个文件来加载配置。
具体读哪些文件，取决于你使用的是哪种 Shell，以及当前是 login session、interactive session 还是两者兼有，整个过程可能相当复杂。
想深入了解的话，[这里](https://blog.flowblok.id.au/2013-02/shell-startup-scripts.html)有一篇很好的参考资料。
...
意思是把 `$PATH` 设为「当前值加上新路径」，并让所有子进程继承。
这样子进程就能找到 `path/to/append` 下的程序。
...
可以用 Shell 内建的 `alias` 命令自定义别名。
alias 本质上是另一个命令的简写，Shell 在真正执行前会自动把它替换掉。
比如在 bash 里，alias 大致是这样：
...
应该把什么放进 dotfiles？
可以通过在线文档或 [man page](https://en.wikipedia.org/wiki/Man_page) 了解工具支持哪些配置。也可以在网上搜索相关文章，作者通常会分享自己偏好的定制方式。还有一种方法：直接看别人的 dotfiles。GitHub 上有很多 [dotfiles 仓库](https://github.com/search?o=desc&q=dotfiles&s=stars&type=Repositories)，最热门的在 [这里](https://github.com/mathiasbynens/dotfiles)（不过不建议不加判断地整套照抄）。
