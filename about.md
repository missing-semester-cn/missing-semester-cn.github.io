...
然而有一个至关重要的主题却很少被专门讲授，而是留给学生们自己去探索。 这部分内容就是：工具的精通使用。
...
如何使用命令别名、脚本和构建系统来自动化执行通用重复的任务。不再总是从文档中拷贝粘贴
...
我们将教你如何通过拉取请求（Pull Request）来为他人的项目贡献代码。
...
如何使用SSH密钥高效地连接和维持远程服务器会话，并实现终端复用。不再为了仅执行个别命令
